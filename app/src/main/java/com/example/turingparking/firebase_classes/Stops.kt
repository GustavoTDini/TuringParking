...
class Stops(var userId: String){
    ...
    override fun toString(): String {
        return "Stops(userId='$userId', id='$id', carId='$carId', parkingId='$parkingId', spotId='$spotId', preferential=$preferential, electric=$electric, occupied=$occupied, reserved=$reserved, cancelled=$cancelled, active=$active, finalized=$finalized, cost=$cost, timeOfCheckIn=$timeOfCheckIn, timeOfReserve=$timeOfReserve, timeOfReserveCancel=$timeOfReserveCancel, timeOfLeave=$timeOfLeave, estimatedTimeOfArrive=$estimatedTimeOfArrive, rating=$rating, latitude=$latitude, longitude=$longitude)"
    }
